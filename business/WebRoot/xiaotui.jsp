		...
		<div class="dvcontent">
			...
			<div>
				...
				<div class="tabs">
					<div class="hd" >
						<ul id="menu">
							<li style="box-sizing: initial;-webkit-box-sizing: initial;" class="on">查看销售退货单</li>
						</ul>
					</div>
					<div class="bd">
						<ul style="display: block;padding: 20px;">
							<li>
								...
								<div id="dv1">
									...
									<a href="<%=Const.ROOT%>/sreturn/sreturnList?currentPageNo=${pageInfo.firstPage}" aria-label="Next">首页</a>	
									<c:if test="${pageInfo.pageNum==1}">
							<a href="javascript:void(0)" aria-label="Previous">上一页</a>
									</c:if>
									<c:if test="${pageInfo.pageNum>1}">
							<a href="<%=Const.ROOT%>/sreturn/sreturnList?currentPageNo=${pageInfo.prePage}" aria-label="Previous">上一页</a>
									</c:if>
							...
							<a href="javascript:void(0)" aria-label="Next">下一页</a>
									...
									<c:if test="${pageInfo.pageNum<pageInfo.pages}">
							<a href="<%=Const.ROOT%>/sreturn/sreturnList?currentPageNo=${pageInfo.nextPage}" aria-label="Next">下一页</a>
									</c:if>
								<a href="<%=Const.ROOT%>/sreturn/sreturnList?currentPageNo=${pageInfo.lastPage}" aria-label="Next">尾页</a>	
				...
				var btn_delete = function(id) {
					$.jq_Confirm({
						...
						btnOkClick: function() {
							$.ajax({
								type: "post",
								url: "<%=Const.ROOT%>/sreturn/delete",
								data: { id: id },
								success: function(data) {
								//alert(data)
									if(data > 0) {
								
										location.href="<%=Const.ROOT%>/sreturn/sreturnList";
									}
								}
							});
						}
